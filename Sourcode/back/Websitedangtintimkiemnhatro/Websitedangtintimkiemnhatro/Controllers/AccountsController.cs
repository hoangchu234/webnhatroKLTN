...
namespace Websitedangtintimkiemnhatro.Controllers
{
    ...
    public class AccountsController : ControllerBase
    {
        ...
        public async Task<IActionResult> PutAccount(int id, Account account)
        {
            ...
            var accountFind = await _context.Accounts.FirstOrDefaultAsync(a => a.Id == id);
            // hash password
        ...
        // PUT: api/Accounts/5
        [HttpPut]
        [Route("PutUpdatePhone/{id}")]
        public async Task<IActionResult> PutUpdatePhone(int id, Account account)
        {
            if (id != account.Id)
            {
                return BadRequest();
            ...
            var accountFind = await _context.Accounts.FirstOrDefaultAsync(a => a.Id == id);
            accountFind.Phone = account.Phone;
            _context.Entry(accountFind).State = EntityState.Modified;
            try
            {
                await _context.SaveChangesAsync();
        ...
        // GET: api/Accounts/GetAccountPhoneCheck
        [HttpGet]
        [Route("GetAccountPhoneCheck/{id}")]
        public async Task<ActionResult<Account>> GetAccountPhoneCheck(string id)
        {
            var account = _context.Accounts.Where(a => a.Id.ToString() == id).FirstOrDefault();
        ...
        // POST: api/Accounts/SigninUser
        //phone,password
        [HttpPost]
        [Route("SigninUser")]
        public async Task<ActionResult<Account>> SigninUser(Account account)
        {
            string phone = account.Phone;
            var accountfind = _context.Accounts.Include(m => m.User).Include(m => m.Employee).Where(b => b.Phone == phone).FirstOrDefault();
            return accountfind;
